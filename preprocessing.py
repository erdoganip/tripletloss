...
class PreProcessing:
    ...
    def __init__(self,data_src):
        ...
        with open('images_train.npy', 'wb') as f:
            np.save(f, self.images_train)
        print("Labels train :", self.labels_train.shape)
        with open('labels_train.npy', 'wb') as f:
            np.save(f, self.labels_train)
        print("Images test  :", self.images_test.shape)
        with open('images_test.npy', 'wb') as f:
            np.save(f, self.images_test)
        print("Labels test  :", self.labels_test.shape)
        with open('labels_test.npy', 'wb') as f:
            np.save(f, self.labels_test)
        print("Unique label :", self.unique_train_label)
        with open('unique_train_label.npy', 'wb') as f:
            np.save(f, self.unique_train_label)
        ...
        for directory in os.listdir(self.data_src):
            try:
                for pic in os.listdir(os.path.join(self.data_src, directory)):
                    ...
                    img = cv2.imread(os.path.join(self.data_src, directory, pic),cv2.COLOR_BGR2RGB )
    ...
    def get_triplets(self):
        ...
        #    a = np.random.choice(self.map_train_label_indices[label_l])
        #    p = np.random.choice(self.map_train_label_indices[label_l])
