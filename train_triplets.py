...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_integer('batch_size', 64, 'Batch size.') #512
flags.DEFINE_integer('train_iter', 1000, 'Total training iter')
flags.DEFINE_integer('step', 50, 'Save after ... iteration')
flags.DEFINE_float('learning_rate','0.0001','Learning rate')
flags.DEFINE_float('momentum','0.99', 'Momentum') #9.9
flags.DEFINE_string('model', 'conv_net', 'model to run')
flags.DEFINE_string('data_src', './data_repository/lfw_2/', 'source of training dataset')

#'./data_repository/geological_similarity/'
...
if __name__ == "__main__":
    ...
    train_step = tf.train.MomentumOptimizer(FLAGS.learning_rate, FLAGS.momentum, use_nesterov=True).minimize(loss, global_step=global_step)
